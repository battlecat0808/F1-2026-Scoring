...
st.set_page_config(page_title="2026 F1 Scoring Ultimate (24 Drivers)", page_icon="🏎️", layout="wide")
    ...
    if st.button("🚀 提交成績", use_container_width=True, type="primary"):
        ...
        for d, r in inputs.items():
            ...
            if v == 'R': processed[d] = 'R'
                ...
                try:
                    ...
                    else: err = True; err_msg = f"排名 {n} 重複或超出範圍(1-24)！"
        ...
        if err: st.error(f"❌ {err_msg}")
            ...
            if r_type == "正賽":
                ...
                # 紀錄衝刺賽歷史
                st.session_state.sprint_history.append({"race_after": curr_mark, "results": sprint_pts})
...
with tab_wdc:
    def get_avg_pos(ranks):
        ...
        processed = [r if isinstance(r, int) else 26 for r in ranks] # DNF 權重改為 26
...
with tab_wcc:
    ...
    for t in TEAM_CONFIG.keys():
        ...
        all_ranks = []
        for d_s in ds: all_ranks.extend([r if isinstance(r, int) else 26 for r in d_s["ranks"]])
    ...
    if st.session_state.race_no > 0:
        ...
        for d in sorted_drivers:
            ...
            for i, r in enumerate(s["ranks"], 1):
                row[f"Rd.{i}"] = 26 if r == 'R' else r
            pos_data.append(row)
        df_pos = pd.DataFrame(pos_data)

        def style_ranks_text(val):
            if not isinstance(val, (int, float)): return ''
            if val == 26: return 'color: #FF4B4B; font-weight: bold'
