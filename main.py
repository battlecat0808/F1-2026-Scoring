            ...
            if v == 'R': processed[d] = 'R'
                ...
                try:
                    ...
                    if 1 <= n <= 24 and n not in used_ranks: 
                        ...
                        err = True; err_msg = f"排名 {n} 重複或超出範圍！"
                except: 
                    err = True; err_msg = "格式不對！"
...
with tab_wdc:
    ...
    for i, (n, s) in enumerate(d_sort, 1):
        ...
        d_data.append([trend, i, s['no'], n, s['team'], s['points'], avg_p if avg_p != 99.0 else "N/A", f"{s['p1']}/{s['p2']}/{s['p3']}", s['dnf']])
...
with tab_pos:
    if st.session_state.race_no > 0:
        ...
        for d in sd:
            s = st.session_state.stats[d]
            row = {"車手": d, "車隊": s['team']}
            # 【視覺優化】直接將 'R' 轉換為字串 "DNF"，更符合真實 F1 視覺
            for i, r in enumerate(s["ranks"], 1): 
                row[f"Rd.{i}"] = "DNF" if r == 'R' else r
            pos_data.append(row)
            
        df_pos = pd.DataFrame(pos_data)
        
        def style_ranks_text(val):
            if val == "DNF": return 'color: #FF4B4B; font-weight: bold;'
